...
class UnsupervisedTargetMetrics(cbck.Callback):
    ...
    def on_epoch_end(self, epoch, *args, **kwargs):
        if epoch % self.interval == 0:
            ...
            X_val_2d = np.reshape(self.X_val, (tf.shape(self.X_val)[0], -1))
...
def get_callbacks(validation_data, save_path, data_name: str, track_loss: str, interval: int = 5, other_cbcks: str = "",
    ...
    checkpoint = cbck.ModelCheckpoint(filepath=save_fd + "models/checkpoints/epoch-{epoch}", save_best_only=True, save_weights_only=True,
