...
import models.actpc.model_utils as model_utils
...
class ACTPC(tf.keras.Model):
    ...
    def _initialise_enc_pred(self, data, val_data, epochs=100, batch_size=64,
        ...
        enc_pred_vars = [var for var in self.trainable_variables if "actor" in var.name.lower() or "critic" in var.name.lower()]
        ...
        for epoch in range(epochs):
            ...
            y_true = tf.argmax(tf.concat(all_y_true, axis=0), axis=1)
            y_pred = tf.argmax(tf.concat(all_y_pred, axis=0), axis=1)
            acc = tf.reduce_mean(tf.cast(y_true == y_pred, tf.float32)).numpy()
            # Print result and update tracker
            print("End of epoch %d ---------------- \n Training loss: %.4f  Validation loss %.4f  ACC %.4f" % (
                epoch, epoch_loss / step_train_, val_loss / step_val_, acc))
    ...
    def _initialise_sel(self, data, val_data, epochs=100, batch_size=64, patience_epochs=200):
        ...
        sel_vars = [var for var in self.trainable_variables if "selector" in var.name.lower()]
        ...
        for epoch in range(epochs):  # Iterate through epochs
            ...
            for val_step_, (x_batch, clus_batch) in enumerate(val_dataset):
                ...
                all_y_true.append(clus_batch)
                all_y_pred.append(clus_val_pred) 
            ...
            # Print result and update tracker
            print("End of epoch %d - \n Training loss: %.4f  Validation loss %.4f  ACC %.4f" % (
                epoch, epoch_loss / step_, val_loss / val_step_, acc))
...
class Model(ACTPC):
    ...
    def build_model(self, data_info):
        ...
        # Load Checkpoint
        callbacks, run_num = model_utils.get_callbacks((X_val, y_val), data_info['save_path'], data_name=data_name, track_loss="l_crit",
    ...
    def train(self, data_info, save_path, lr: float = 0.001, epochs_init: int = 10, epochs: int = 100, bs: int = 32,
        ...
        - "epochs_init": number of epochs to train initialisation (default = 10)
    ...
    def test_model(self, X):
        # the learning process easily collapses and the model always predicts 0 outcome
        # so we need to search for the best thresh
