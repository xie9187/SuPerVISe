...
import models.camelot.model_utils as model_utils
from models.model_blocks import MLP, AttentionRNNEncoder, LSTMEncoder
...
class CAMELOT(tf.keras.Model):
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape(watch_accessed_variables=False, persistent=True) as tape:
            ...
            # compute losses
            l_crit = model_utils.l_crit(y, y_pred, weights=loss_weights)
    ...
    def _initialise_enc_pred(self, data, val_data, epochs=100, batch_size=64, patience_epochs=200, training_rate=0.001):
        ...
        input_dataset = tf.data.Dataset.from_tensor_slices(data).shuffle(100, seed=self.seed).batch(batch_size)
        val_dataset = tf.data.Dataset.from_tensor_slices(val_data).shuffle(100, seed=self.seed).batch(batch_size)
        ...
        for epoch in range(epochs):
            ...
            for step_, (x_batch, y_batch) in enumerate(input_dataset):
                ...
                optimiser.apply_gradients(zip(enc_pred_grad, enc_pred_vars))
    ...
    def _initialise_iden(self, data, val_data, epochs=100, batch_size=64, patience_epochs=200, training_rate=0.001):
        ...
        for epoch in range(epochs):  # Iterate through epochs
            ...
            # Check if result hasn't improved for 2 epochs
            if epoch > patience_epochs and epoch > 20 and val_loss >= self.iden_loss_tracker.iloc[-patience_epochs-20:-21, -1].min():
                break
...
class Model(CAMELOT):
    ...
    def build_model(self, data_info):
        ...
        # Load Checkpoint
        callbacks, run_num = model_utils.get_callbacks((X_val, y_val), data_info['save_path'], data_name=data_name, track_loss="L_pred",
                                                       other_cbcks=cbck_str, patience_epochs=patience_epochs,
                                                       early_stop=True, lr_scheduler=True, tensorboard=False)
    ...
    def train(self, data_info, save_path, lr_init: float = 0.001, lr: float = 0.001, epochs_init_1: int = 100, epochs_init_2: int = 100, epochs: int = 100,
        ...
        if os.path.exists(model_path + '.index'):
            ...
            train_data = tf.data.Dataset.from_tensor_slices(train_data).shuffle(100).batch(bs)
            val_data = tf.data.Dataset.from_tensor_slices(val_data).shuffle(100).batch(bs)
    ...
    def test_model(self, X, b_size):
        all_risk, all_clus_probs = [], []
        for idx in range(0, len(X), b_size):
            x = X[idx:idx+b_size]
            risk = self.Predictor(self.Encoder(x)).numpy()[:, 1]
            clus_probs = self.compute_pis(x)
            all_risk.append(risk)
            all_clus_probs.append(clus_probs)
        risk = np.concatenate(all_risk, axis=0)
        clus_probs = np.concatenate(all_clus_probs, axis=0)
        return risk, clus_probs
